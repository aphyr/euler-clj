...
(defn p11
  ...
  (let [grid "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
        ...
        grid (mapv (fn [row]
                     (mapv #(Integer. %) (split-by \space row)))
                   (split-by \newline grid))
        width  (apply min (map count grid))
        height (count grid)
        l 4
        drifts (mapcat (fn [direction]
                  (grids/drifts [height width] direction 4))
                [[1 0] [1 1] [0 1] [-1 1]])]
    ; We could be sneaky and walk the grid iteratively, multiplying and
    ; dividing an accumulator, but this feels more general.
    (apply max
           (map (fn [drift]
...
(defn p14
  ...
  []
  (apply max-key (collatz-sequence-length*) 
         (take-while (partial > 1000000) ordinals)))
